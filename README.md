...
To be more precice, Jupyter notebook is <a href="https://jupyter.org/install](https://en.wikipedia.org/wiki/Integrated_development_environment)" target="_blank" style="outline: none;" rel="noopener">on its own</a>


an cloud IDE that is used to create Jupyter documents [.ipnyb] that can be created and shared with live codes. Also, it is a web-based interactive computational environment. The Jupyter notebook can support various languages that are popular in data science such as Python, Julia, Scala, R, etc.1
